...
from src.services.training_plan.pace.performance_calculator import calculate_median_easy_pace
...
def test_median_easy_pace(user_id: str):
    ...
    try:
        ...
        for i, run in enumerate(runs, 1):
            ...
            print(f"{i:2d}. {run.start_date.date()} | {run.conv_distance:5.2f} mi | "
        ...
        if median_pace:
            ...
            print(f"  Avg: {format_pace(sum(paces)/len(paces))} ({sum(paces)/len(paces):.1f} sec/mi)")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print()
        print("Example:")
        print('  python scripts/test_median_easy_pace.py "2e1c2581-619c-4a2a-a8b4-4dfc265e7789"')
        sys.exit(1)
