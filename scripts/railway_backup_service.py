...
def cleanup_old_backups(backup_path: str, retention_days: int) -> int:
    ...
    print(f"🧹 Cleaning up backups older than {retention_days} days (before {cutoff_date.date()})")
    ...
    for backup_file in backup_dir.glob("backup_*.sql"):
        try:
            ...
            if file_time < cutoff_date:
                ...
                backup_file.unlink()
                deleted_count += 1
                print(f"  🗑️  Deleted: {backup_file.name} ({file_size:,} bytes, {file_time.date()})")
...
def main():
    ...
    try:
        ...
        for backup in backups[:5]:  # Show last 5
            ...
            print(f"  • {backup_path_obj.name} ({size:,} bytes, {mtime.strftime('%Y-%m-%d %H:%M:%S')})")
        if len(backups) > 5:
