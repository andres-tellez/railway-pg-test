...
export default function PostOAuthSuccess() {
  ...
  useEffect(() => {
    ...
    async function fetchAthleteAndSync() {
      ...
      if (!code || authed !== "true") {
        console.error("❌ Missing code or not a real auth redirect");
        setStatus("error");
        return;
      }
      ...
      try {
        ...
        const { access_token, refresh_token } = JSON.parse(rawText);

        console.log("🔐 Tokens received:", { access_token, refresh_token });

        if (!access_token) {
          throw new Error("No access_token received from /auth/callback");
        }
