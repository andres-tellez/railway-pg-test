...
| `phase_kpi_priority` | `src/services/phase/phase_priority.py::compute_phase_kpi_priority_for_week(phases)` + pure `resolve_week_phase(phases)` + `phase_kpi_priority_for_phase(phase)` + `Phase` / `PhaseKpi` / `PhaseKpiPriorityResult` types (**landed 2026-04-21, Phase A item 6**) | `src/services/plan/weekly_plan.py::build_weekly_plan_payload` (week-level `phase_kpi_priority` block; consumed verbatim by `/api/plan/current-week` and by the LLM `get_weekly_plan` tool for all past / current / future temporalities), future Phase B `get_phase_analysis`, §19.4 prompt builder (top 1–2 emphasis), §19.1 coach response validator (no-override contract) |
