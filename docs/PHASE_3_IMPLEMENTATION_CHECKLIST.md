...
| `phase_kpi_priority` | `src/services/phase/phase_priority.py::compute_phase_kpi_priority_for_week(phases)` + pure `resolve_week_phase(phases)` + `phase_kpi_priority_for_phase(phase)` + `Phase` / `PhaseKpi` / `PhaseKpiPriorityResult` types (**landed 2026-04-21, Phase A item 6**) | `/api/plan/current-week` (week-level `phase_kpi_priority` block), future Phase B `get_weekly_plan` + `get_phase_analysis`, §19.4 prompt builder (top 1–2 emphasis), §19.1 coach response validator (no-override contract) |
