...
| `SMARTCOACH_RUN_RECAP_WEEK_VOLUME_BUNDLE` | `1` | When on (default), recap prefetch adds **`week_volume_context`** (`this_week` / `last_week`: `run_count`, `total_mi_display`, labels) from **`aggregate_runs_in_range`** (ISO Mon–Sun, activity local dates). `0`/`false`/`no`/`off` skips that call. |
