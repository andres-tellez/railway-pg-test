        ...
        with get_session() as session:
            if use_orchestrator:
                generation_mode = os.getenv(
                    "TRAINING_PLAN_GENERATION_MODE", "one_pass"
                ).lower()
                if generation_mode == "three_pass":
                    ...
                    def weeks_until(race_str: str) -> int:
                        try:
                            rd = race_str
                            if isinstance(rd, (datetime, date)):
                                dt = rd
                            else:
                                dt = datetime.fromisoformat(race_str)
                    ...
                    ctx = {
                        ...
                        "plan_request": plan_dict,
                        "weeks": recom_weeks,
                        "current_weekly_mileage": insights.get(
                            "current_fitness", {}
                        ).get("weekly_mileage", 0),
                        "longest_recent_run": insights.get("current_fitness", {}).get(
                            "longest_run", 0
                        ),
                        "experience": (
                            plan_dict.get("marathon_experience") or ""
                        ).lower(),
                    ...
                    tp = ThreePassOrchestrator(llm)
                    result = tp.generate_longrun_first(ctx)
                    if not result.get("valid"):
                        raise ValueError(
                            "Generated plan failed validation in three-pass mode"
                        )
                    validation = {
                        "valid": True,
...
def create_plan_draft_route():
    ...
    try:
        ...
        generation_mode = os.getenv("TRAINING_PLAN_GENERATION_MODE", "one_pass").lower()
        ...
        with get_session() as session:
            if generation_mode == "three_pass":
                ...
                ctx = {
                    ...
                    "experience": (
                        plan_request.get("marathon_experience") or ""
                    ).lower(),
                ...
                tp = ThreePassOrchestrator(llm)
                lr_only = True
                if use_longrun_first and lr_only:
                    ...
                    if lr_series:
                        ...
                        if len(lr_series) >= 3:
                            ...
                            ok = all(
                                abs(last3[i] - targets[i]) <= 1.0 for i in range(3)
                            )
                            if not ok:
                                violations.append(
                                    {
                                            ...
                                            for idx in recovery_week_indices:
                                                ...
                                                if pass3_result.get(
                                                    ...
                                                    weeks_simple[idx]["workouts"] = (
                                                        pass3_result["weeks"][0][
                                                            "workouts"
                                                        ]
                                                    )

                                                    # Verify progression: Mon < Wed < Thu
                                                    workouts = weeks_simple[idx][
                                                        "workouts"
                                                    ]
                                                    mon_miles = next(
                                                        (
                        ...
                        try:
                            ...
                            if rd:
                                ...
                                for i, week in enumerate(weeks_simple):
                                    ...
                                    month_name = week_start.strftime(
                                        "%b"
                                    )  # Jan, Feb, etc.
                                ...
                                race_metadata = {
                                    ...
                                    "start_date_label": start_date.strftime(
                                        "%b %d, %Y"
                                    ),
                                }
                        except Exception as e:
                ...
                result = (
                    tp.generate_longrun_first(ctx)
                    if use_longrun_first
                    else tp.generate(ctx)
                )
                if result.get("valid"):
                    validation = {
                        "valid": True,
                    ...
                    generated_plan = result["validated_plan"]
                else:
                    # Provide draft with violations for UI
                    validation = {
                        "valid": False,
                        "violations": result.get("violations", []),
                    }
                    generated_plan = result.get("draft")

                draft = {"generated_plan": generated_plan, "validation": validation}
            else:
                orchestrator = TrainingPlanOrchestratorService.create_default()
