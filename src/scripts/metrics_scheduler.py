#!/usr/bin/env python
"""
Scheduler for weekly maintenance tasks - runs every Sunday at 9:15 AM Central Time.
...
def should_run_scheduled_tasks():
    """Check if it's Sunday at 9:15 AM Central Time."""
    ...
    # It's Sunday (weekday 6) and at 9:15 AM Central Time (hour 9, minute 15)
    if now.weekday() == 6 and now.hour == 9 and now.minute == 15:
        return True
...
def run_all_scheduled_tasks():
    ...
    logger.info("🚀 Starting weekly scheduled tasks (Sunday 9:15 AM Central Time)...")
...
def main():
    ...
    print(f"[SCHEDULER] Schedule: Sunday at 9:15 AM Central Time", flush=True)
    print(
    ...
    logger.info(
        "🕐 Weekly scheduler started - waiting for Sunday at 9:15 AM Central Time..."
    )
    logger.info(
        f"💡 Scheduled tasks will run automatically every Sunday at 9:15 AM Central Time"
    )
            ...
            if should_run:
                ...
                if now.minute % 10 == 0:
                    logger.info(
                        ...
                        f"weekday={now.weekday()}, hour={now.hour}, minute={now.minute}) - next run: Sunday at 9:15 AM Central Time"
                    )
