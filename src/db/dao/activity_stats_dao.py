...
class ActivityStatsDAO:
    ...
    @staticmethod
    def get_recent_activities(session: Session, athlete_id: int, days: int) -> List[Activity]:
    ...
    @staticmethod
    def get_activities_by_date_range(session: Session, athlete_id: int, start_date: datetime, end_date: datetime) -> List[Activity]:
        stmt = select(Activity).where(
            Activity.athlete_id == athlete_id,
            Activity.type == "Run",
    ...
    @staticmethod
    def get_total_distance(session: Session, athlete_id: int, start_date: datetime, end_date: datetime) -> float:
    ...
    @staticmethod
    def get_average_pace(session: Session, athlete_id: int, days: int) -> Optional[float]:
        cutoff = datetime.utcnow() - timedelta(days=days)
        stmt = select(
    ...
    @staticmethod
    def get_longest_run(session: Session, athlete_id: int, days: int) -> Optional[Activity]:
        cutoff = datetime.utcnow() - timedelta(days=days)
        stmt = select(Activity).where(
            Activity.athlete_id == athlete_id,
            Activity.type == "Run",
            Activity.start_date >= cutoff
        ).order_by(Activity.distance.desc()).limit(1)
        return session.scalar(stmt)

    @staticmethod
    def get_fastest_run(session: Session, athlete_id: int, days: int) -> Optional[Activity]:
        cutoff = datetime.utcnow() - timedelta(days=days)
        stmt = select(Activity).where(
            Activity.athlete_id == athlete_id,
            Activity.type == "Run",
            Activity.start_date >= cutoff,
            Activity.distance > 0
        ).order_by((Activity.moving_time / (Activity.distance / 1000)).asc()).limit(1)
        return session.scalar(stmt)

    @staticmethod
    def get_weekly_summary(session: Session, athlete_id: int, past_weeks: int = 4) -> List[Dict]:
    ...
    @staticmethod
    def get_hr_zone_summary(session: Session, athlete_id: int, days: int) -> Dict[str, float]:
        cutoff = datetime.utcnow() - timedelta(days=days)
        stmt = select(
    ...
    @staticmethod
    def get_treadmill_vs_outdoor_stats(session: Session, athlete_id: int, days: int) -> Dict[str, int]:
        cutoff = datetime.utcnow() - timedelta(days=days)
    ...
    @staticmethod
    def get_runs_by_weekday(session: Session, athlete_id: int, days: int) -> Dict[str, int]:
        cutoff = datetime.utcnow() - timedelta(days=days)
        stmt = select(extract("dow", Activity.start_date), func.count()).where(
            Activity.athlete_id == athlete_id,
            Activity.type == "Run",
            Activity.start_date >= cutoff
    ...
    @staticmethod
    def get_time_of_day_stats(session: Session, athlete_id: int, days: int) -> Dict[str, int]:
        cutoff = datetime.utcnow() - timedelta(days=days)
    ...
    @staticmethod
    def get_trend_metrics(session: Session, athlete_id: int, metric: str, window_size: int = 7) -> List[Dict]:
