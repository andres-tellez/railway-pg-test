"""
Training Profile Normalizer Service
====================================
...
def normalize_training_profile(
    readiness: Dict[str, Any], user_profile: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    if age_factor < 1.0:
        ...
        print(f"\nAge Adjustments:")
        print(
            f"  • Ramp limit: {cfg['ramp_limit_pct']*100:.0f}% → {adjusted_ramp*100:.0f}%"
        )
        print(
            f"  • Threshold sessions: {cfg['threshold_sessions']} → {adjusted_threshold}"
        )
    ...
    if weeks_to_race < 8:
        ...
        print(
            f"  • {weeks_to_race} weeks to race - reducing target mileage by {(1-time_pressure_factor)*100:.0f}%"
        )
    ...
    normalized_profile = {
        ...
        "recommended_training_days": (
            4 if category == "Low-Base" else 5 if category == "Stable-Base" else 6
        ),
    ...
    print(
        f"  • Quality sessions: {normalized_profile['threshold_sessions_per_week']}/week"
    )
...
def get_jack_daniels_phase_parameters(
    phase: str, normalized_profile: Dict[str, Any]
) -> Dict[str, Any]:
    """
