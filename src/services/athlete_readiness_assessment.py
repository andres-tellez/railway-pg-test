"""
Athlete Readiness Assessment Service
=====================================
...
2. Renaming to `athlete_readiness_service.py` to follow naming conventions
3. Updating imports if integrated
"""
...
def assess_runner_readiness(
    activities: List[Dict[str, Any]], user_profile: Dict[str, Any], race_date: date
) -> Dict[str, Any]:
    """
    ...
    for act in recent_activities:
        ...
        if week_start not in weekly_mileage:
            weekly_mileage[week_start] = {"total_miles": 0, "run_count": 0}
    ...
    print(
        f"  • Weekly totals: {[round(m, 1) for m in sorted(weekly_totals, reverse=True)]}"
    )
    print(f"  • Base mileage (avg): {base_mileage:.1f} mi/week")

    # ---- 4️⃣ Longest recent run ----
    longest_run = (
        max([act["distance"] for act in recent_activities]) if recent_activities else 0
    )
    print(f"  • Longest run: {longest_run:.1f} miles")

    # ---- 5️⃣ Consistency score ----
    # Combine frequency AND mileage stability
    weeks_with_3_runs = sum(
        1 for week in weekly_mileage.values() if week["run_count"] >= 3
    )
    ...
    if weekly_totals and mean_miles > 0:
        week_deviation = sum(abs(w - mean_miles) for w in weekly_totals) / len(
            weekly_totals
        )
    ...
    print(
        f"  • Weeks with 3+ runs: {weeks_with_3_runs} / {total_weeks} ({frequency_score:.1f}%)"
    )
    ...
    if long_run_ratio < 0.2:
        ...
        long_run_ratio_score = max(
            60, 100 - (long_run_ratio - 0.35) * 200
        )  # penalty if too long

    print(f"\n📊 Component Scores:")
    print(
        f"  • Base mileage score: {base_mileage_score:.1f} / 100 (target: {target_mileage} mi/week)"
    )
    print(f"  • Consistency score: {consistency_score:.1f} / 100")
    print(
        f"  • Zone balance score: {zone_balance_score:.1f} / 100 (easy range: 50-70%)"
    )
    print(
        f"  • Long run ratio: {long_run_ratio:.1%} (score: {long_run_ratio_score:.1f})"
    )
    ...
    return {
        ...
        "debug": {
            ...
            "week_deviation": (
                round(week_deviation, 1) if weekly_totals and base_mileage > 0 else 0
            ),
...
def _fallback_readiness_assessment(
    weeks_to_race: float, user_profile: Dict[str, Any]
) -> Dict[str, Any]:
    """
