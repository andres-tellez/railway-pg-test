...
from src.services.training_plan.v2.marathon.workout_types_v2 import (
    ...
    get_non_long_shares,
    get_slot_counts,
    get_min_non_long_day,
    get_long_run_share_range,
)
...
def calculate_workout_distribution(
    ...
    *,
    ...
    shares = get_non_long_shares(n, config)
    assert (
    ...
    min_each = get_min_non_long_day(config)
    ...
    # ENDURANCE
    miles[run_days[far_idx]] = max(min_each, largest)

    # EASY
    miles[run_days[close_idx]] = max(min_each, smallest)

    # STEADY (distribute middles: closer gets smaller, farther gets larger)
    for amt, i in zip(sorted(steadies), steady_positions):
        miles[run_days[i]] = max(min_each, amt)
    ...
    if over > 0:
        ...
        for d in trim_order:
            if over <= 0:
                break
            reducible = max(0, miles[d] - min_each)
...
def validate_workout_distribution(
    ...
    *,
    ...
    min_non_long = get_min_non_long_day(config)
    # V5: Each non-long ≥ minimum
    for idx in non_long_idxs:
        day = run_days[idx]
        if schedule[day]["miles"] < min_non_long:
            errors.append(
    ...
    lo, hi = get_long_run_share_range(freq, config)
