...
def find_insertion_points(
    weeks: List[Dict[str, Any]], extra_weeks: int, config: RaceDistanceConfig
) -> List[int]:
    ...
    if not build_peak_indices:
        ...
        taper_start = max(0, len(weeks) - config.taper_weeks)
...
def create_recovery_week(
    previous_week: Dict[str, Any], config: RaceDistanceConfig
) -> Dict[str, Any]:
    ...
    max_lr_reduction = max(1.0, config.target_peak_miles * 0.15)
    lr_reduction = min(max_lr_reduction, original_lr * 0.20)
    new_lr = round((original_lr - lr_reduction) * 2) / 2.0  # Round to 0.5

    # Ensure minimums
    min_long_run = max(2.0, config.target_peak_miles * 0.3)
    new_lr = max(min_long_run, new_lr)
...
def insert_recovery_weeks(
    ...
    insertion_points = find_insertion_points(weeks, extra_weeks, config)
    ...
    for insert_after_idx in insertion_points:
        if insert_after_idx < len(modified_weeks):
            ...
            recovery_week = create_recovery_week(previous_week, config)
...
def _get_local_today(timezone_str: str) -> date:
    try:
        ...
        try:
            ...
            logger.warning(
                "Default timezone '%s' unavailable via zoneinfo. Using UTC fallback.",
                DEFAULT_TIMEZONE,
            )
...
def apply_recovery_week_insertion_if_needed(
    ...
    *,
    ...
    if extra_weeks > 0:
        modified_weeks = insert_recovery_weeks(weeks, extra_weeks, config)
