...
class PaceConfig:
    ...
    STEADY_MIN_OFFSET: float = -30.0  # Faster than Easy, slower than Marathon
    STEADY_MAX_OFFSET: float = (
        -20.0
    )  # Must be < EASY_MIN_OFFSET (-15) to ensure Steady max < Easy min
    ...
    CALIBRATION_EASY_MIN: float = 630.0  # 10:30/mile
    CALIBRATION_EASY_MAX: float = 690.0  # 11:30/mile
    CALIBRATION_STEADY_MIN: float = (
        610.0  # 10:10/mile (between Marathon 600 and Easy min 630)
    )
    CALIBRATION_STEADY_MAX: float = (
        625.0  # 10:25/mile (between Marathon 600 and Easy min 630)
    )
