...
def convert_metrics(data: dict, fields: list[str]) -> dict:
    ...
    if "elevation" in fields:
        conversions["conv_elevation_feet"] = meters_to_feet(
            safe_float(data.get("elevation"))
        )
    if "average_speed" in fields:
        conversions["conv_avg_speed"] = mps_to_min_per_mile(
            safe_float(data.get("average_speed"))
        )
    if "max_speed" in fields:
        conversions["conv_max_speed"] = mps_to_min_per_mile(
            safe_float(data.get("max_speed"))
        )
    if "moving_time" in fields:
        conversions["conv_moving_time"] = format_seconds_to_hms(
            safe_int(data.get("moving_time"))
        )
    if "elapsed_time" in fields:
        conversions["conv_elapsed_time"] = format_seconds_to_hms(
            safe_int(data.get("elapsed_time"))
        )
    return conversions
