...
def refresh_materialized_views(session, athlete_id=None):
    ...
    if success_count == total_count:
        logger.info(f"🎉 All materialized views refreshed successfully ({success_count}/{total_count})")
    else:
...
def refresh_views_after_activity_sync(session, athlete_id, activities_synced=0):
    ...
    if activities_synced > 0:
        logger.info(f"🔄 Refreshing materialized views after syncing {activities_synced} activities for athlete {athlete_id}")
        return refresh_materialized_views(session, athlete_id)
    else:
        logger.info(f"⏭️ Skipping materialized view refresh - no new activities synced for athlete {athlete_id}")
