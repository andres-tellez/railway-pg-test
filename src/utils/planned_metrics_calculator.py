...
def get_planned_miles_for_current_week(session, user_id):
    ...
    try:
        ...
        plan_result = session.execute(text("""
        ...
        workouts_result = session.execute(text("""
            SELECT miles 
            FROM plan_workouts 
        ...
        logger.info(f"📊 Found {len(workouts_result)} planned workouts totaling {total_planned_miles} miles")
