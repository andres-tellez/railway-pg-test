...
def _enforce_plan_creation_response_guardrails(
    ...
    *,
    ...
    Runs on **model-authored text only**. Deterministic intake fallback copy from
    ``_natural_plan_intake_fallback_question`` must not pass through this helper — it
    collapses whitespace and breaks multi-line recaps.

    Callers apply activity preambles *after* this step so overview paragraphs are not
    counted toward the sentence cap.
