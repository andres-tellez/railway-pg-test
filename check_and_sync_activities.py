...
def main():
    ...
    try:
        ...
        recent_activities = ActivityStatsDAO.get_recent_activities(
        ...
        for activity in recent_activities:
            print(
                f"  - {activity.start_date.strftime('%Y-%m-%d %H:%M')}: {activity.name} ({activity.distance:.2f} km)"
            )
        ...
        if len(recent_activities) == 0:
            ...
            synced_count = service.ingest_between(
                start_date, end_date, max_activities=50
            )
