"""Unit tests for tempo/threshold pace-progress corridor authority."""
...
def test_inside_corridor_is_green():
    ...
    mid = (fast + slow) / 2.0
    assert (
        classify_threshold_pace_progress(pace_sec_per_mi=fast, goal_aligned_z3_pace=z3)
        == "green"
    )
    assert (
        classify_threshold_pace_progress(pace_sec_per_mi=slow, goal_aligned_z3_pace=z3)
        == "green"
    )
    assert (
        classify_threshold_pace_progress(pace_sec_per_mi=mid, goal_aligned_z3_pace=z3)
        == "green"
    )
...
def test_outside_corridor_bilateral_bands():
    z3 = _goal_z3()
    fast, slow = tempo_corridor_sec(z3)
    cfg = DEFAULT_PACE_PROGRESS_THRESHOLD_CONFIG
...
def test_reference_uses_goal_z3_display():
    z3 = _goal_z3()
    ref = build_threshold_pace_progress_reference(z3)
...
def test_zones_include_bilateral_tiers():
    z3 = _goal_z3()
    zones = build_threshold_pace_progress_zones_chart(z3)
