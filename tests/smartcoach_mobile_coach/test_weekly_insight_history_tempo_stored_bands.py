"""Tempo weekly-history chart GYOR bands use stored kpi_snapshot at write time."""

from __future__ import annotations

import json
from datetime import date
from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from src.smartcoach_mobile_coach.execution_analytics.tempo_segment import (
    TempoSegmentPaceResult,
)
from src.smartcoach_mobile_coach.runner_profile.models import (
    HrZoneBand,
    PaceZoneBand,
    RunnerZoneProfileData,
)
from src.smartcoach_mobile_coach.runner_profile.recommendations.training_phase_resolver import (
    TrainingPhaseResolution,
)
from src.smartcoach_mobile_coach.weekly_insights_service import (
    _compute_tempo_system_pipeline,
    calendar_week_containing,
    get_weekly_insight_history,
)

USER_ID = "00000000-0000-0000-0000-000000000001"


def _calibrated_profile() -> RunnerZoneProfileData:
    from datetime import datetime, timezone

    return RunnerZoneProfileData(
        user_id=USER_ID,
        calibrated=True,
        computed_at=datetime.now(timezone.utc),
        hrmax_used=185,
        resting_hr_used=50,
        zone_method="karvonen",
        hr_z1=HrZoneBand(100, 115),
        hr_z2=HrZoneBand(120, 145),
        hr_z3=HrZoneBand(146, 160),
        hr_z4=HrZoneBand(161, 175),
        hr_z5=HrZoneBand(176, 185),
        pace_z2=PaceZoneBand(low_sec=600, high_sec=630, display="10:00-10:30/mi"),
        pace_z3=PaceZoneBand(low_sec=570, high_sec=590, display="9:30-9:50/mi"),
        pace_z4=PaceZoneBand(low_sec=530, high_sec=540, display="8:50-9:00/mi"),
        pace_source="performance",
        pace_computed_at=datetime.now(timezone.utc),
    )


def _phase_resolution() -> TrainingPhaseResolution:
    return TrainingPhaseResolution(
        phase="Base",
        source="test",
        week_start=date(2026, 4, 7),
    )


def _mock_tempo_rollups():
    cal_week_start, _ = calendar_week_containing(date.today())
    segment = TempoSegmentPaceResult(
        tempo_segment_pace_min_per_mi=8.75,
        tempo_segment_avg_hr_bpm=155.0,
        tempo_segment_pace_source="z3_splits",
        tempo_segment_split_count=4,
        tempo_segment_confidence="high",
    )

    def _inner(session, user_id, week_windows, athlete_id):
        return {ws: (1, segment) for ws, _we in week_windows}

    return _inner, cal_week_start


def test_compute_tempo_system_pipeline_stores_gyor_bands():
    target_tempo = PaceZoneBand(low_sec=570, high_sec=590, display="9:30-9:50/mi")
    target_hr_z3 = HrZoneBand(146, 160)
    tempo_kpis = {
        "tempo_run_count": 1,
        "tempo_segment_pace_min_per_mi": 8.75,
        "tempo_segment_avg_hr_bpm": 155.0,
        "tempo_segment_confidence": "high",
        "effort_stability_min_per_mi": None,
    }

    pipeline = _compute_tempo_system_pipeline(
        tempo_kpis,
        prior=None,
        target_tempo_pace=target_tempo,
        target_hr_z3=target_hr_z3,
    )

    assert pipeline["bands"]["tempo_pace_progress"] is not None
    assert pipeline["bands"]["tempo_hr_progress"] is not None


@patch(
    "src.smartcoach_mobile_coach.weekly_insights_service._fetch_tempo_week_rollups_batch",
)
@patch(
    "src.smartcoach_mobile_coach.runner_profile.service.resolve_current_training_phase",
    return_value=_phase_resolution(),
)
@patch(
    "src.smartcoach_mobile_coach.insights_chart_authority.get_runner_profile",
    return_value=_calibrated_profile(),
)
@patch(
    "src.smartcoach_mobile_coach.insights_chart_authority.get_active_or_most_recent_plan",
    return_value=SimpleNamespace(target_time="3:40:00", race_distance="Marathon"),
)
@patch(
    "src.smartcoach_mobile_coach.weekly_insights_service.get_primary_athlete_id",
    return_value=12345,
)
def test_weekly_history_tempo_bands_use_stored_kpi_snapshot(
    _mock_athlete,
    _mock_plan,
    _mock_profile,
    _mock_phase,
    mock_tempo_batch,
):
    """Stored GYOR bands win over on-read reclassification when targets would differ."""
    mock_fn, cal_week_start = _mock_tempo_rollups()
    mock_tempo_batch.side_effect = mock_fn

    snapshot = {
        "systems": {
            "tempo": {
                "bands": {
                    "tempo_pace_progress": "red",
                    "tempo_hr_progress": "orange",
                }
            }
        }
    }

    session = MagicMock()
    session.execute.return_value.fetchall.return_value = [
        SimpleNamespace(
            week_start=cal_week_start,
            hr_drift_pct=2.0,
            hr_drift_band="green",
            z2_pace_min_per_mi=9.5,
            z2_pace_band="green",
            efficiency=1.2,
            efficiency_band="green",
            easy_avg_hr=142.0,
            easy_avg_hr_band="green",
            kpi_snapshot=snapshot,
        )
    ]

    out = get_weekly_insight_history(session, USER_ID, weeks=1)

    tempo_points = out["systems"]["tempo"]["weekly_data"]
    assert len(tempo_points) == 1
    point = tempo_points[0]
    assert point["tempo_pace_progress_band"] == "red"
    assert point["tempo_hr_progress_band"] == "orange"


def test_tempo_gyor_bands_from_insight_row_parses_json_string():
    from src.smartcoach_mobile_coach.weekly_insights_service import (
        _tempo_gyor_bands_from_insight_row,
    )

    row = SimpleNamespace(
        kpi_snapshot=json.dumps(
            {
                "systems": {
                    "tempo": {
                        "bands": {
                            "tempo_pace_progress": "yellow",
                            "tempo_hr_progress": "green",
                        }
                    }
                }
            }
        )
    )
    assert _tempo_gyor_bands_from_insight_row(row) == ("yellow", "green")
