"""Tempo display authority on /weekly-history matches /zones recommendation builders."""
...
def _expected_tempo_authority_from_recs(profile: RunnerZoneProfileData):
    ...
    authority = build_insights_tempo_chart_authority_payload(recs)
    ...
    assert (
        authority["pace_target_display"] == tp["tempo_pace_progress"]["target_display"]
    )
    assert authority["hr_target_display"] == tp["tempo_hr_progress"]["target_display"]
    assert authority["insights_tempo_banner"] == zones_payload["insights_tempo_banner"]
    assert (
        authority["insights_tempo_banner"]["subtitle"] == INSIGHTS_TEMPO_BANNER_SUBTITLE
    )
    return authority
...
def test_tempo_chart_authority_payload_matches_zones(
    _mock_plan, _mock_profile, _mock_phase
):
    profile = _calibrated_profile_with_paces()
    _expected_tempo_authority_from_recs(profile)
...
def test_weekly_history_tempo_authority_without_chart_history(
    _mock_athlete,
    _mock_plan,
    _mock_profile,
    _mock_phase,
    ...
    profile = _calibrated_profile_with_paces()
    expected = _expected_tempo_authority_from_recs(profile)
    ...
    assert tempo["pace_target_display"] == expected["pace_target_display"]
    assert tempo["hr_target_display"] == expected["hr_target_display"]
    assert tempo["insights_tempo_banner"] == expected["insights_tempo_banner"]
    assert tempo["weekly_data"] == []
...
def test_weekly_history_tempo_authority_with_easy_history(
    _mock_athlete,
    _mock_plan,
    _mock_profile,
    _mock_phase,
