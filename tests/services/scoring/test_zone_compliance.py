"""Unit tests for src.services.scoring.zone_compliance.
...
class TestZoneMetricsForTypeCanonical:
    ...
    def test_recovery_uses_only_zone_1_as_target(self):
        # Recovery target=(1,), acceptable=[1..2], so above=Z3+Z4+Z5.
        m = zone_metrics_for_type(self.DIST, RUN_TYPE_RECOVERY)
        assert m.in_target == 10.0
        assert m.pct_below == 0.0
        assert m.pct_above == 70.0

    def test_steady_uses_zones_2_and_3_as_target(self):
        # Steady target=(2,3), acceptable=[2..3], so below=Z1, above=Z4+Z5.
        m = zone_metrics_for_type(self.DIST, RUN_TYPE_STEADY)
        assert m.in_target == 50.0
        assert m.pct_below == 10.0
        assert m.pct_above == 40.0
...
class TestZoneMetricsEdgeCases:
    ...
    def test_rounding_to_two_decimal_places(self):
        ...
        m = zone_metrics_for_type(dist, RUN_TYPE_STEADY)
        # Steady target=(2,3): Z2+Z3 = 66.6666... rounded to 66.67
        assert m.in_target == 66.67
        assert m.pct_below == 33.33
        assert m.pct_above == 0.0
    ...
    def test_unknown_run_type_raises_key_error(self):
        with pytest.raises(KeyError):
            zone_metrics_for_type({1: 100.0}, "not_a_real_run_type")
