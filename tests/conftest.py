...
def seed_test_data(test_db_session):
    if not test_db_session.query(Athlete).filter_by(athlete_id=1).first():
        test_db_session.add(Athlete(athlete_id=1, first_name="Test", last_name="Athlete"))
    ...
    if not test_db_session.query(Activity).filter_by(activity_id=SAMPLE_ACTIVITY_JSON["activity_id"]).first():
