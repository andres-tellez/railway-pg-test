...
class PerformanceBenchmark:
            ...
            try:
                ...
                if method.upper() == "GET":
                    response = requests.get(
                        f"{self.base_url}{endpoint}", headers=headers or {}, timeout=30
                    )
    ...
    def benchmark_multiple_endpoints(
        self, endpoints: List[Tuple[str, str, Dict, Dict]], iterations: int = 10
    ) -> Dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for endpoint, method, data, headers in endpoints:
                future = executor.submit(
                    self.benchmark_endpoint, endpoint, method, data, headers, iterations
                )
    ...
    def _generate_summary(self, results: Dict) -> Dict:
        ...
        return {
            ...
            "slowest_endpoint": (
                max(results.items(), key=lambda x: x[1].get("avg_response_time", 0))[0]
                if results
                else None
            ),
        }
    ...
    def print_report(self, report: Dict):
        ...
        print(
            f"Overall Average Response Time: {summary['overall_avg_response_time']:.3f}s"
        )
        ...
        for endpoint, data in report["results"].items():
            if "error" in data:
                ...
                print(
                    f"   Min/Max: {data['min_response_time']:.3f}s / {data['max_response_time']:.3f}s"
                )
                print(f"   Error Rate: {data['error_rate']:.1f}%")
                print(
                    f"   Successful Requests: {data['successful_requests']}/{data['iterations']}"
                )
